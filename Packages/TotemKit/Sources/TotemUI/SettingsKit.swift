...
/// A profile shortcut: glyph over caption on its own rounded tile.
///
/// The tile is the button. An earlier version put a tinted circle behind the
/// glyph and left the tap target invisible, which read as decoration sitting
/// on a card rather than as something you press.
        ...
        Button(action: action) {
            VStack(spacing: 8) {
                ...
                Text(title)
                    ...
                    .lineLimit(1)
                    // Long titles shrink rather than truncate: four tiles
                    // across a phone leaves little room at large text sizes.
                    .minimumScaleFactor(0.75)
            }
            .foregroundStyle(Color.accentColor)
            .frame(maxWidth: .infinity)
            .padding(.vertical, 14)
            .background(Theme.tile, in: RoundedRectangle(cornerRadius: 18, style: .continuous))
            .contentShape(.rect)
        }
...
/// Presses the whole tile rather than dimming the glyph, so the feedback
/// matches the shape being touched.
...
/// A row of quick actions, evenly spaced.
public struct QuickActionRow<Content: View>: View {
...
#Preview {
    List {
        ...
        Section {
            UsageBar(segments: [
                .init(id: "a", fraction: 0.5, color: Theme.cobalt),
                .init(id: "b", fraction: 0.3, color: Theme.orchid),
                .init(id: "c", fraction: 0.2, color: Theme.lagoon)
            ])

            UsageLegendRow(color: Theme.cobalt, title: "Photos", detail: "62 MB")
        }
