...
struct DevicesView: View {
    ...
    private static let otherSessions: [DeviceSession] = [
        DeviceSession(
            id: "mac",
            name: "MacBook Air",
            detail: "Orbit Desktop 1.0 - 2 hours ago",
...
struct ChatFoldersView: View {
    ...
    private func tint(for filter: ChatListFilter) -> Color {
        switch filter {
        case .all: Color(.systemBlue)
        case .unread: Color(.systemGreen)
        case .groups: Color(.systemOrange)
        case .channels: Color(.systemPurple)
        }
    }
}
...
struct HelpView: View {
    ...
    private static let questions: [Question] = [
        Question(
            ...
            answer: "On this device only. Orbit is a design study, so there is no server behind it yet and nothing leaves your phone."
        ),
        Question(
            id: "receipts",
        ...
        Question(
            id: "storage",
            question: "Why is Orbit using storage?",
